    ...
    public:
        ...
        llvm::Value* Codegen(){
            ...
            if(Type=="int")
                ...
                return ReportError::ErrorV("invalid data type of Global Variable");
            }
            for (list <idList *>::iterator it = idlist -> begin(); it != idlist -> end(); it++) {
                if((*it)->isArray==false){
                    llvm::GlobalVariable *gv = new llvm::GlobalVariable(*TheModule,ty,false,llvm::GlobalValue::CommonLinkage,0,(*it)->variable);
                    //llvm::GlobalVariable *test = TheModule->getNamedGlobal((*it)->variable);
                }
                else{
                    cout<<"its array !! :-o "<<(*it)->variable<<endl;
                    llvm::GlobalVariable *gv = new llvm::GlobalVariable(*TheModule,llvm::PointerType::getUnqual(llvm::ArrayType::get(ty,(*it)->num_elements)),false,llvm::GlobalValue::CommonLinkage,0,(*it)->variable);
                    //llvm::ArrayType::get(ty,(*it)->num_elements);
...
class idListArr: public idList{
    ...
    public:
        ...
        void evaluate(int level){
            ...
            cout<<"ID:"<<variable<<"[]"<<endl;
            printLevel(level+1);
            cout<<"INT_LITERAL: "<<num_elements<<endl;
        }
...
class location_node : public expr_node{
    ...
    public:
        ...
        llvm::Value* Codegen(){
            ...
            llvm::Value *Val  = llvm::ConstantInt::get(llvm::getGlobalContext(), llvm::APInt(32,1));
            if(expn_ != NULL){
                llvm::Value *Val = expn_->Codegen();
                if(Val==0){
                    return ReportError::ErrorV("array index is not valid\n");
                }
                //llvm::GlobalVariable *test = TheModule->getNamedGlobal(string(id_));
                //if(test){
                  //  cout<<"global variable is present\n";
                //}
                //llvm::Value* V llvm::ConstantInt::get(llvm::getGlobalContext(), llvm::APInt(32,1));
                //return V;
                //llvm::Value *x0 = Builder.CreateExtractValue(Builder.CreateLoad(test),Val);
            }
...
class operator_node: public expr_node{
    ...
    public:
        ...
        llvm::Value* Codegen(){
            ...
            string op(optr);
            if(!strcmp(optr,"+")){
                V =  Builder.CreateAdd(L, R, "addInstruct");
            }
            else if(!strcmp(optr,"-")){
                V = Builder.CreateSub(L, R, "subInstruct");
            }
            else if(!strcmp(optr,"*")){
                V = Builder.CreateMul(L, R, "mulInstruct");
            }
            else if(!strcmp(optr,"/")){
                V = Builder.CreateUDiv(L, R, "divInstruct");
            }
            else if(!strcmp(optr,"%")){
                V = Builder.CreateURem(L, R, "remInstruct");
            }
            else if(!strcmp(optr,">")){
                V = Builder.CreateICmpUGT(L, R, "cmpgtInstruct");
            }
            else if(!strcmp(optr,">=")){
                V = Builder.CreateICmpUGE(L, R, "cmpgeInstruct");
            }
            else if(!strcmp(optr,"<")){
                V = Builder.CreateICmpULT(L, R, "cmpltInstruct");
            }
            else if(!strcmp(optr,"<=")){
                V = Builder.CreateICmpULE(L, R, "cmpleInstruct");
            }
            else if(!strcmp(optr,"==")){
                V = Builder.CreateICmpEQ(L, R, "cmpisEqualInstruct");
            }
            else if(!strcmp(optr,"!=")){
                V = Builder.CreateICmpNE(L, R, "cmpisNotEqualInstruct");
            }
    ...
    public:
        ...
        llvm::Value* Codegen(){
            ...
            if(!strcmp(assignop_->id_,"=")){
                ...
                rightVal = Builder.CreateAdd(Builder.CreateLoad(Variable, string(lnode_->id_)),rightVal,"addEqualTo");
            }
            else if(!strcmp(assignop_->id_,"-=")){
                if(debug)
                    cout<<"minus equality operator\n";
                rightVal = Builder.CreateSub(Builder.CreateLoad(Variable, string(lnode_->id_)),rightVal,"subEqualTo");
            }
            if(debug){
                cout<<"storing\n";
                rightVal->dump();
            ...
            return Builder.CreateStore(rightVal, Variable);
        }        
